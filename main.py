...
def formulario_productos():
    try:
        ...
        if username is None:
            return render_template("login.html")
        if token == usuario[9] and usuario[10] == 1 and usuario[11] == True:
            # Obtén el estado del formulario, si no se proporciona, asume 'vigentes'
            estado = request.args.get('estado', 'vigentes')

            if estado == 'vigentes':
                productos = controlador_producto.obtener_productos_formateado()
            elif estado == 'no_vigentes':
                productos = controlador_producto.obtener_productos_no_vigentes_formateado()
            else:
                # Trata cualquier otro valor como 'vigentes'
                productos = controlador_producto.obtener_productos_formateado()
...
def formulario_clientes():
    try:
        ...
        if token == usuario[9] and usuario[10] == 1 and usuario[11] == True:
            ...
            clientes = controlador_cliente.obtener_clientesFormateado(
                nombre_busqueda)
