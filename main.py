...
def api_obtener_comprobantes():
    ...
    for comprobante in comprobantes:
        objComprobante = clase_comprobante.Comprobante(comprobante[0], comprobante[1], comprobante[2], comprobante[3], comprobante[4], comprobante[5])
...
def api_actualizar_comprobante():
    ...
    controlador_comprobante.actualizar_comprobante(fechaE, monto_total, tipo_comprobante, pedido_id, metodo_id, numero_boleta)
    respuesta["data"] = listadiccs
    respuesta["code"] = 3
...
def api_insertar_comprobante():
    ...
    pedido_id = request.json["pedido_id"]
    metodo_id = request.json["metodo_id"]
    controlador_comprobante.insertar_comprobante(fechaE, monto_total, tipo_comprobante, pedido_id, metodo_id)
    respuesta["data"] = listadiccs
    respuesta["code"] = 3
...
def api_obtener_comprobante_por_numero():
    ...
    listadiccs = []
    numero_boleta = request.json["numero_boleta"]
    comprobante = controlador_comprobante.obtener_comprobante_por_numero(numero_boleta)
    objComprobante = clase_comprobante.Comprobante(comprobante[0], comprobante[1], comprobante[2], comprobante[3], comprobante[4], comprobante[5])
    listadiccs.append(objComprobante.obtenerObjetoSerializable())
    respuesta["data"] = listadiccs
    respuesta["code"] = 4
...
def api_actualizar_metodo_pago():
    ...
    controlador_metodos_pago.actualizar_metodo_pago(nombre, descripcion, estado, id)
    respuesta["data"] = listadiccs
    respuesta["code"] = 2
...
def api_insertar_producto():
    ...
    mascota_id = request.json["mascota_id"]
    link_imagen = request.json["link_imagen"]
    controlador_producto.insertar_producto(nombre,descripcion,precio,stock,categoria_id,mascota_id,link_imagen)
    respuesta["data"] = listadiccs
    respuesta["code"] = 3
    respuesta["message"] = "Guardado Correcto"
    return jsonify(respuesta)
...
def api_insertar_pedido():
    ...
    controlador_pedido.insertar_pedidos(fecha_pedido, estado_pedido, usuario_id)
    respuesta["data"] = listadiccs
    respuesta["code"] = 2
...
def api_actualizar_pedido():
    ...
    estado_pedido = request.json["estado_pedido"]
    usuario_id = request.json["usuario_id"]
    controlador_pedido.actualizar_pedido(fecha_pedido, estado_pedido, usuario_id, pedido_id)
    respuesta["data"] = listadiccs
    respuesta["code"] = 3
...
def metodo_actualizar_producto():
    ...
    controlador_producto.actualizar_producto_sinImagen(nombre,descripcion,precio,stock,estado,categoria_id,mascota_id,producto_id)
    return redirect("/productos")
